...
def build_extra(df, engine):
    ...
    # KPI aggregates per platform AND per time-range, so BOTH filters change the cards
    ...
    def _kmetrics(sub):
        n = int(len(sub))
        rt = pd.to_numeric(sub["rating"], errors="coerce")
        rt = rt[(rt >= 1) & (rt <= 5)].dropna()          # stars only (excl. reddit upvotes)
        return {"reviews": n,
                "avg_rating": round(float(rt.mean()), 1) if len(rt) else None,
                "positive_pct": round(float((sub["sentiment"] == "positive").mean()), 3) if n else 0,
                "frustrated_pct": round(float((sub["sentiment"] == "frustrated").mean()), 3) if n else 0,
                "theme_count": int(sub["category"].nunique())}

    _ranges = {"all": None, "2024": "2024", "2025": "2025", "2026": "2026"}
    per_platform, per_platform_range = {}, {}
    for _k, _sub in _plats.items():
        _y26 = int((_sub["_year"] == "2026").sum())
        _y25 = int((_sub["_year"] == "2025").sum())
        per_platform[_k] = {**_kmetrics(_sub),
                            "trend_reviews": round((_y26 - _y25) / _y25, 3) if _y25 else None}
        per_platform_range[_k] = {}
        for _rk, _rmin in _ranges.items():
            if _rmin is None:
                _rsub = _sub
            elif _rk == "2026":
                _rsub = _sub[_sub["_year"] == "2026"]
            else:
                _rsub = _sub[_sub["_year"].fillna("") >= _rmin]
            per_platform_range[_k][_rk] = _kmetrics(_rsub)

    return {
        "generated_from": "discovery_insights_dataset.csv (frozen v1) + engine_output.json",
        "per_platform": per_platform,
        "per_platform_range": per_platform_range,
